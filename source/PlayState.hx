...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		PlayStateChangeables.Middle = FlxG.save.data.middlescroll;
		...
		if (executeModchart)
			PlayStateChangeables.Middle = false;			
		#end
		#if !cpp
		...
		switch(stageCheck)
		{
			...
			case 'alley':
				{
					defaultCamZoom = 0.9;
					curStage = 'alley';
					var bg:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('modded stages/whitty/whittyBack', 'shared'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
	
					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('modded stages/whitty/whittyFront', 'shared'));
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					stageFront.antialiasing = true;
					stageFront.scrollFactor.set(0.9, 0.9);
			...
			case 'ballisticAlley':
			{
				...
				var bgTex = Paths.getSparrowAtlas('modded stages/whitty/BallisticBackground', 'shared');
		...
		if (SONG.gfVersion == null) {
			switch(storyWeek)
			{
				...
				case 8: gfCheck = 'picoSpeaker';
				case 9: gfCheck = 'gf-whitty';
			}
		...
		difficultinfoTxt = new FlxText(4,healthBarBG.y + 50,0,CoolUtil.difficultyFromInt(storyDifficulty), 16);
		difficultinfoTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		difficultinfoTxt.scrollFactor.set();
		add(difficultinfoTxt);
		
		if (PlayStateChangeables.useDownscroll)
			difficultinfoTxt.y = FlxG.height * 0.9 + 45;
		...
		var colorBar1 = dominantColor(iconP1);
		var colorBar2 = dominantColor(iconP2);
		...
		difficultinfoTxt.cameras = [camHUD];
		...
		if (curStage == 'ballisticAlley')
			{
				funneEffect = new FlxSprite(-600, -200).loadGraphic(Paths.image('modded stages/whitty/thefunnyeffect', 'shared'));
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...keyAmmo[mania])
		{
			...
			if (PlayStateChangeables.Middle && player == 0)
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					case 'normal':
					    babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets-shaggy');
			...
			if (PlayStateChangeables.Middle)
				babyArrow.x -= 320;
	...
	override public function update(elapsed:Float)
	{
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			if (luaModchart.getVar("showOnlyStrums",'bool'))
			{
				...
				difficultinfoTxt.visible = false;
	...
	{	
		...
		if (offsetTesting)
		{
			    FlxG.sound.playMusic(Paths.music('classic'));
			offsetTesting = false;
