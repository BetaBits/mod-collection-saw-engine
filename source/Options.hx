...
class MEMOption extends Option
{
	...
	public override function press():Bool
	{
		...
		(cast (Lib.current.getChildAt(1), Main)).toggleMem(FlxG.save.data.memory);
		display = updateDisplay();
		return true;
	}
...
class Watermark extends Option
{
	...
	public override function press():Bool
	{
		...
		(cast (Lib.current.getChildAt(1), Main)).toggleMark(FlxG.save.data.watermark);
		display = updateDisplay();
		return true;
	}
	...
	private override function updateDisplay():String
	{
		return "Watermark " + (!FlxG.save.data.watermark ? "off" : "on");
	}
}
